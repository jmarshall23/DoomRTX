...
void idPhysics_RigidBody_RestorePState(idRestoreGame* savefile, rigidBodyPState_t& state) {
	...
	state.localAxis.FixDenormals();
	state.pushVelocity.SubVec3(0).FixDenormals();
	state.pushVelocity.SubVec3(1).FixDenormals();
	state.externalForce.FixDenormals();
	state.externalTorque.FixDenormals();
	state.i.position.FixDenormals();
	state.i.orientation.FixDenormals();
	state.i.linearMomentum.FixDenormals();
	state.i.angularMomentum.FixDenormals();
}
...
void idPhysics_RigidBody::SetMaster(idEntity* master, const bool orientated) {
	...
	if (master) {
		...
		current.localOrigin = vec3_origin;
		current.localAxis = mat3_identity;
...
void idPhysics_RigidBody::ReadFromSnapshot(const idBitMsgDelta& msg) {
	...
	current.localAxis.FixDenormals();
	current.pushVelocity.SubVec3(0).FixDenormals();
	current.pushVelocity.SubVec3(1).FixDenormals();
	current.externalForce.FixDenormals();
	current.externalTorque.FixDenormals();
