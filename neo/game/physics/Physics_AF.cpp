...
void idAFConstraint_Spring::SetLimit(const float minLength, const float maxLength) {
	assert(minLength >= 0.0f && maxLength >= 0.0f && (minLength == 0.0f || maxLength == 0.0f || maxLength >= minLength));
...
void idAFConstraint_Suspension::Evaluate(float invTimeStep) {
	...
	gameLocal.clip.Translation(trace, start, end, wheelModel, axis, CONTENTS_SOLID, NULL);
...
idEntity* idPhysics_AF::SetupCollisionForBody(idAFBody* body) const {
	...
	if (!selfCollision || !body->fl.selfCollision || af_skipSelfCollision.GetBool()) {
		...
		// unlink all bodies so the body only collides with the world and external entities
		for (i = 0; i < bodies.Num(); i++) {
		...
		// link only bodies that can participate in self collision
		for (i = 0; i < bodies.Num(); i++) {
			if (bodies[i]->fl.selfCollision) {
				bodies[i]->clipModel->Link(gameLocal.clip, self, bodies[i]->clipModel->GetId(), bodies[i]->current->worldOrigin, bodies[i]->current->worldAxis);
			}
			else {
				bodies[i]->clipModel->Unlink();
			}
		...
		// unlink any bodies attached with no-collision constraints
		for (i = 0; i < body->constraints.Num(); i++) {
			if (!body->constraints[i]->fl.noCollision) {
				continue;
			}
...
bool idPhysics_AF::EvaluateContacts(void) {
	...
	for (i = 0; i < bodies.Num(); i++) {
		...
		dir.Zero();
		dir.SubVec3(0) = body->current->spatialVelocity.SubVec3(0) + current.lastTimeStep * force.SubVec3(0);
...
void idPhysics_AF::SetupContactConstraints(void) {
	...
	// make sure enough contact constraints are allocated, and release unused ones
...
bool idPhysics_AF::Evaluate(int timeStepMSec, int endTimeMSec) {
	...
	// make sure all clip models are unlinked in case they were linked for self collision
