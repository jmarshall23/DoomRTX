...
const float PM_AIRFRICTION = 0.2f;
const float PM_WATERFRICTION = 1.0f;
const float PM_FLYFRICTION = 6.0f;
...
ETQW-style command scaling helper.  The original Doom 3 CmdScale only
ignored upmove while walking; air movement also has no vertical control,
so jump/crouch input should not reduce horizontal air wishspeed.
==================
*/
static float PM_CmdScale(const usercmd_t& cmd, const float playerSpeed, const bool noVertical) {
	int		max;
	float	total;
	int		forwardmove;
	int		rightmove;
	...
	return (float)playerSpeed * max / (127.0f * total);
}
...
==================
*/
static bool PM_IsWalkablePlane(const idVec3& normal, const idVec3& gravityNormal) {
	return (normal * -gravityNormal) >= MIN_WALK_NORMAL;
}
...
Given a walkable plane normal, adjust the vertical component so the vector lies
in the plane without changing the horizontal direction.  This is the portable
part of ETQW's idPhysics_Player::AdjustVertically and replaces several slope
speed-loss cases caused by plain ProjectOntoPlane.
==================
*/
static idVec3 PM_AdjustVertically(const idVec3& normal, const idVec3& _in, const idVec3& gravityNormal) {
	idVec3 out;
	idVec3 in = _in;
	idVec3 up = -gravityNormal;
	idVec3 velocityPlane = in.Cross(up);

	// same horizontal direction and in the walkable plane
	...
	// maintain an overclip on the vertical component so rounding problems do not
	// put us back into solids. For normal Doom 3 gravity this matches ETQW's z test.
	float upComponent = out * up;
	idVec3 vertical = up * upComponent;
	idVec3 horizontal = out - vertical;
	if (upComponent >= 0.0f) {
		vertical *= OVERCLIP;
	}
	else {
...
float idPhysics_Player::CmdScale(const usercmd_t& cmd) const {
	// Preserve Doom 3's public behavior for callers that do not opt in to the
	// explicit noVertical path: walking ignores upmove, non-walking modes keep it.
	return PM_CmdScale(cmd, playerSpeed, walking);
}
...
bool idPhysics_Player::SlideMove(bool gravity, bool stepUp, bool stepDown, bool push) {
	...
	if (gravity) {
		...
		if (groundPlane) {
			...
			if (PM_IsWalkablePlane(groundTrace.c.normal, gravityNormal)) {
	...
	for (bumpcount = 0; bumpcount < numbumps; bumpcount++) {
		...
		// if we are allowed to step up, only do stair stepping for non-walkable hits
		...
		for (i = 0; i < numplanes; i++) {
			if ((trace.c.normal * planes[i]) > 0.999f) {
				if (PM_IsWalkablePlane(trace.c.normal, gravityNormal)) {
					current.velocity = PM_AdjustVertically(trace.c.normal, current.velocity, gravityNormal);
				}
				else {
					// clip into the trace normal just in case this normal is almost but not exactly
					// the same as the groundTrace normal, then nudge out along the normal.
		...
		for (i = 0; i < numplanes; i++) {
			...
			if (PM_IsWalkablePlane(planes[i], gravityNormal)) {
				clipVelocity = PM_AdjustVertically(planes[i], current.velocity, gravityNormal);
				endClipVelocity = PM_AdjustVertically(planes[i], endVelocity, gravityNormal);
			}
			else {
			...
			for (j = 0; j < numplanes; j++) {
				...
				if (PM_IsWalkablePlane(planes[j], gravityNormal)) {
					clipVelocity = PM_AdjustVertically(planes[j], clipVelocity, gravityNormal);
					endClipVelocity = PM_AdjustVertically(planes[j], endClipVelocity, gravityNormal);
				}
				else {
