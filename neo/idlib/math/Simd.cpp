...
// Record start time
inline void StartRecordTime(Timestamp &val) {
	int cpu_info[4];
	__cpuid(cpu_info, 0); // Serialize instructions with CPUID
	val = __rdtsc();       // Read timestamp counter
}

// Record end time
inline void StopRecordTime(Timestamp& val) {
	int cpu_info[4];
	__cpuid(cpu_info, 0); // Serialize instructions with CPUID
	val = __rdtsc();       // Read timestamp counter
}
