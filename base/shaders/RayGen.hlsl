...
HitInfo FirePrimaryRay() {
  ...
  gLightOutput[launchIndex] = payload.lightColor;
...
[shader("raygeneration")] void RayGen() {
	...
	else if(hit.worldOrigin.w == STAT_MIRROR /*|| hit.worldOrigin.w == STAT_GLASS*/)
	{		 
          float2 dims = float2(DispatchRaysDimensions().xy);
		  float2 d = (((launchIndex.xy + 0.5f) / dims.xy) * 2.f - 1.f);
		  float4 target = mul(projectionI, float4(d.x, d.y, 1, 1));  
		  ...
		  TraceRay(
		   ...
		   if(hit.worldOrigin.w == STAT_MIRROR)
		   {
		   ...
		   else if(hit.worldOrigin.w == STAT_GLASS)
		   {
				gLightOutput[launchIndex].xyz = lerp(gLightOutput[launchIndex], float4((payload.colorAndDistance.rgb * payload.lightColor.rgb), 1.f), 0.8);
		   }
	}
	...
	{		 
		  ...
		  TraceRay(
			...
			gOutput[launchIndex].xyz = gOutput[launchIndex] * float4(payload.colorAndDistance.rgb, 1.f) * 4;
			gLightOutput[launchIndex].xyz = gLightOutput[launchIndex] * float4((payload.lightColor.rgb), 1.f) * 4;
	}
